...
const AddColumn = ({ reloadSpaceContent }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/columns/${params.uid}/${params.spaceId}/${params.tableId}/newColumns`,
        initConfig
      );
    ...
    <Card sx={{ maxWidth: 345, marginTop: 2 }}>
      ...
      >
        <Box component="form" sx={style} onSubmit={formik.handleSubmit}>
          ...
          <Button type="submit" variant="contained" disable={isLoading}>
            Envoyer
          </Button>
          <LoadingAnimation
            isLoading={isLoading}
